...
export function getCustomerToStoreWhatsAppUrl(order, currentUser = null) {
  ...
  const addressLine1 = addr?.line1 ? addr.line1.trim() : "";
  const city = addr?.city ? addr.city.trim() : "";
  const state = addr?.state ? addr.state.trim() : "";
  const postalCode = addr?.postalCode ? addr.postalCode.trim() : "";
  ...
  msg += `\nDelivery Address:\n`;
  if (addressLine1) msg += `${addressLine1}\n`;
  if (city) msg += `${city}\n`;
  if (state || postalCode) {
    msg += `${state}${postalCode ? ` - ${postalCode}` : ""}\n`;
  }
