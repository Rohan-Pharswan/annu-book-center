...
export function getEmailConfig() {
  ...
  const from = (process.env.EMAIL_FROM || "").trim() || "Annu Book Center <onboarding@resend.dev>";
          ...
          <tr>
            <td style="background-color:#f1f5f9;padding:24px;border-top:1px solid #e2e8f0;text-align:center;">
              ...
              <p style="margin:0 0 10px;font-size:12px;color:#64748b;">
                ...
                &bull; ✉️ Reply to this email or contact <a href="mailto:${STORE_CONFIG.primaryPhoneRaw ? 'ordersannubookcenter@gmail.com' : 'ordersannubookcenter@gmail.com'}" style="color:#2563eb;text-decoration:none;">ordersannubookcenter@gmail.com</a>
              </p>
...
export async function sendCustomerOrderConfirmationEmail(orderOrId) {
  ...
  try {
    ...
    // Gracefully handle unconfigured Resend API key
    if (!config.isConfigured) {
      console.log(`[Email Service] Resend API key not configured. Recording status for order ${String(orderId).slice(-6)}.`);
      await Order.findByIdAndUpdate(orderId, {
        confirmationEmailStatus: "not_configured",
        confirmationEmailError: "RESEND_API_KEY environment variable is not configured."
      });
    ...
    if (error) {
      ...
      console.error(`[Email Service] Resend API error sending confirmation for order ${shortId}:`, errorMsg);
    ...
    console.log(`[Email Service] Confirmation email successfully sent to ${recipientEmail} for order #${shortId} (Message ID: ${messageId})`);
